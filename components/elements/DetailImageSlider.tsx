...
const DetailImageSlider = ({ images, className = "" }: DetailImageSliderProps) => {
  ...
  if (!images || images.length === 0) {
    return (
      <div className={`w-full h-[500px] bg-gray-200 dark:bg-gray-700 rounded-lg flex items-center justify-center ${className}`}>
  ...
  return (
    <div className={`relative w-full h-[500px] rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-800 ${className}`}>
      <div 
      ...
      >
        {images.map((image, index) => (
          <div 
            key={`${image}-${index}`} 
            className="h-full relative flex-shrink-0 flex items-center justify-center"
      ...
      {images.length > 1 && (
        <>
          ...
          <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-1.5 px-2 py-1.5 rounded-full bg-black/30 backdrop-blur-sm border border-white/20 z-10">
            {images.map((_, index) => (
              <button
                key={index}
